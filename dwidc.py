...
def main(args):
    ...
    while max_delta >= args.threshold:
        ...
        print_err('  submit {} batches'.format(args.batch_size))
        for i in range(args.batch_size):
            #print_err('  submit {} of {} remaining'.format(i+1, args.batch_size))
            ...
            collected += args.solve_num_reads
            problem_index += 1

        #answers = solve_ising(solver, h, J, **params)
        #print_err('  waiting...')

        solutions_collected = None
        for i, submitted_problem in enumerate(submitted_problems):
            ...
            if solutions_collected != None:
                combis.combine_solution_data(solutions_collected, solutions)
            else:
                solutions_collected = solutions

        if solutions_all != None:
            combis.combine_solution_data(solutions_all, solutions_collected)
        else:
            solutions_all = solutions_collected
        ...
        for solution in solutions_all['solutions']:
            if 'pr' in solution:
                pr = solution['pr']
                solution['pr'] = solution['num_occurrences']/float(collected)
                max_delta = max(max_delta, abs(pr - solution['pr']))
            else:
                solution['pr'] = solution['num_occurrences']/float(collected)
                max_delta = max(max_delta, solution['pr'])
...
def build_cli_parser():
    ...
    parser.add_argument('-th', '--threshold', help='the convegence threshold value', type=float, default=0.001)
    parser.add_argument('-bs', '--batch-size', help='number of batches to collect in each round', type=int, default=5)
